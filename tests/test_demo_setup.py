...
class TestEnsureDemoLawyer:
    def test_creates_lawyer_when_not_exists(self, db):
        ...
        lawyer = ensure_demo_lawyer(db, rut="12345678-9", email="demo@segal.cl", name="Demo Lawyer")
    ...
    def test_idempotent_does_not_duplicate(self, db):
        ...
        ensure_demo_lawyer(db, rut="12345678-9", email="demo@segal.cl")
        ensure_demo_lawyer(db, rut="12345678-9", email="demo@segal.cl")

        count = db.query(Lawyer).filter(Lawyer.rut == "12345678-9").count()
        assert count == 1

    def test_updates_email_on_second_call(self, db):
        """Updates email when the same rut is called again with a different email."""
        ensure_demo_lawyer(db, rut="12345678-9", email="old@segal.cl")
        lawyer = ensure_demo_lawyer(db, rut="12345678-9", email="new@segal.cl")
...
class TestMintToken:
    def test_returns_decodable_jwt(self, db):
        ...
        lawyer = ensure_demo_lawyer(db, rut="12345678-9", email="demo@segal.cl")
