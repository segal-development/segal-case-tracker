"""
PJUD Laboral competency scraper.

Inherits from PJUDBaseScraper and implements laboral-specific logic.
...
# Laboral-specific constants
PJUD_MY_CASES_LABORAL_URL = f"{PJUD_BASE_URL}/misCausas/laboral/consultaMisCausasLaboral.php"
PJUD_MY_CASE_DETAIL_URL = f"{PJUD_BASE_URL}/misCausas/laboral/modal/misCausasLaboral.php"

COMPETENCIA_LABORAL = "4"


# Global selector registry instance (shared across all LaboralScraper instances)
...
def get_selector_registry() -> SelectorRegistry:
    ...
    if _selector_registry is None:
        ...
        # Pre-load laboral selectors
        _selector_registry.load("laboral")
    return _selector_registry


class LaboralScraper(PJUDBaseScraper):
    """
    Scraper for PJUD Laboral portal.
    
    Implements laboral-specific parsing and selectors while inheriting
    ...
    def get_selector(self, name: str) -> str:
        ...
        return self._selector_registry.get("laboral", name)
    
    def get_selector_with_fallbacks(self, name: str) -> List[str]:
        """Get a selector with all fallbacks."""
        return self._selector_registry.get_with_fallbacks("laboral", name)
    
    def _get_competency_config(self) -> CompetencyConfig:
        """Return Laboral competency configuration from registry."""
        registry = self._selector_registry
        
        return CompetencyConfig(
            name="laboral",
            display_name="Laboral",
            competencia_code=registry.get("laboral", "competencia_code"),
            table_body_id=registry.get("laboral", "table_body_id"),
            modal_id=registry.get("laboral", "modal_id"),
            detail_function_name=registry.get("laboral", "detail_function_name"),
            cases_endpoint=registry.get("laboral", "cases_endpoint"),
            detail_endpoint=registry.get("laboral", "detail_endpoint"),
            search_endpoint=registry.get("laboral", "search_endpoint"),
        )
    ...
    async def _fetch_cases_page(
        ...
        """Fetch a single page of laboral cases."""
            ...
            () => {{
                return new Promise((resolve) => {{
                    $.ajax({{
                        ...
                        type: 'POST',
                        data: {{
                            rutMisCauLab: '{rut_num}',
                            dvMisCauLab: '{dv}',
                            tipoMisCauLab: '0',
                            rolMisCauLab: '',
                            anhoMisCauLab: '{year}',
                            fecDesdeMisCauLab: '{fecha_desde}',
                            fecHastaMisCauLab: '{fecha_hasta}',
                            'tipCausaMisCauLab[]': '{tipo_causa}',
                            nombreMisCauLab: '',
                            apePatMisCauLab: '',
                            apeMatMisCauLab: '',
                            pagina: {current_page}
    ...
    def _parse_cases_html(self, html: str) -> List[PJUDCase]:
        """
        Parse HTML response from laboral cases endpoint.
        
        Note: Laboral has 7 columns vs Civil's 8:
        Actions, ROL, Tribunal, Caratulado, Fecha, Estado, Cuaderno
        (no Institucion column)
        """
        cases = []
        ...
        for row in rows:
            ...
            # Extract cell values
            # Laboral columns: Actions, ROL, Tribunal, Caratulado, Fecha, Estado, Cuaderno
            ...
            # Skip cases without valid ROL (Laboral uses patterns like T-123-2026, O-456-2026)
            ...
            cases.append(PJUDCase(
                ...
                institucion=None,  # Laboral doesn't have institucion column
                case_token=token,
            ))
        
        logger.info(f"Parsed {len(cases)} laboral cases from HTML")
        return cases
    
    def _parse_case_detail_html(self, html: str, case_token: str) -> PJUDCaseDetail:
        """Parse laboral case detail HTML including movements."""
        ...
        # ROL: <strong>ROL:</strong> T-7616-2026
        ...
        logger.info(f"Parsed laboral case {rol}: {len(movements)} movements, {len(cuadernos)} cuadernos")
    ...
    # ========================================================================
    # DOCUMENT DOWNLOAD (Laboral-specific endpoints)
    # ========================================================================
    ...
    async def download_document(
        ...
        """
        Download a document PDF from PJUD Laboral.
