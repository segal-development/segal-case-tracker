...
- [ ] Both login paths call real scraper methods; no missing-method errors.
- [ ] One `PJUDSession` model/store; worker finds scraper session by `lawyer_id` in mock tests.
- [ ] No `lawyer_id=0/1` hardcodes; `_get_or_create_lawyer` returns real lawyer.
- [ ] Worker re-authenticates unattended on expiry (Clave Única path) in mock tests.
- [ ] mypy clean; security implication of stored creds documented.
