#!/usr/bin/env python3
"""
Test: v14 - Esperar a que jQuery cargue completamente.
"""
...
async def get_detail_civil(captcha_token: str):
    ...
    async with async_playwright() as p:
        ...
        # 2. Go to misCausas.php
        print()
        print("2. Navegando a /misCausas.php...")
        
        # Use load instead of networkidle to wait for all resources
        await page.goto(f"{base_url}/misCausas.php", wait_until="load")
        await asyncio.sleep(5)  # Extra wait for JS
        
        # Wait for jQuery to be ready
        try:
            await page.wait_for_function("typeof jQuery !== 'undefined' && jQuery.active === 0", timeout=15000)
            has_jquery = True
        except:
        ...
        if not has_jquery:
            ...
            scripts = await page.evaluate("""
                () => Array.from(document.querySelectorAll('script[src]'))
                    .map(s => s.src)
                    .filter(s => s.includes('jquery'))
            """)
            print(f"   Scripts jQuery: {scripts}")
            
            # Try loading jQuery manually
            print("   Inyectando jQuery...")
            await page.add_script_tag(url="https://code.jquery.com/jquery-3.6.0.min.js")
            await asyncio.sleep(2)
            has_jquery = await page.evaluate("typeof jQuery !== 'undefined'")
            print(f"   jQuery después de inyección: {has_jquery}")
        
        if not has_jquery:
            print("   ⚠️ No se pudo cargar jQuery")
            await browser.close()
            return
