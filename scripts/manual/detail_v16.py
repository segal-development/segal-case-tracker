#!/usr/bin/env python3
"""
Test: v16 - Ejecutar misCausas() para cargar el contenido en #contMain.
"""
...
async def get_detail_civil(captcha_token: str):
    ...
    async with async_playwright() as p:
        ...
        # 2. Execute misCausas() function to load the panel
        print()
        ...
        # Check if #contMain has misCausas content
        has_content = await page.evaluate("""
            () => {
                const cont = document.querySelector('#contMain');
                return cont ? cont.innerHTML.length > 1000 : false;
            }
        """)
        print(f"   Contenido cargado: {has_content}")
        ...
        if not tokens:
            print("   Sin tokens")
            with open("screenshots/civil_debug.html", "w") as f:
                f.write(civil_html)
            await browser.close()
            return
